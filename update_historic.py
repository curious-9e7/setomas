...
FUSO_BR = pytz.timezone("America/Sao_Paulo") # Configuração do fuso horário de Brasília


def atualizar_historico_guias(data_alvo: str):
    """
    Varre as guias de uma data específica no banco, baixa o PDF uma única vez
    e atualiza o array de estados, espécies e a flag relevante (TO) em um único push.
    """
    logging.info(f"Buscando guias para atualização histórica na data {data_alvo}...")

    # Define o intervalo do dia inteiro para o filtro de timestamp
    inicio_dia = f"{data_alvo}T00:00:00"
    fim_dia = f"{data_alvo}T23:59:59"

    # Busca as guias do dia alvo
    # Filtro 1 (DB-side): Apenas GF3I, e trazemos as colunas necessárias para o Filtro 2
    resposta = (
        supabase.table('guias_florestais')
        .select('id, numero, tipo, link, num_especie, relevante, rotas_estados, nomes_especies')
        .eq('tipo', 'GF3I') 
        .gte('data_emissao', inicio_dia)
        .lte('data_emissao', fim_dia)
        .order('data_emissao', desc=True)
    ...
    if not guias:
        logging.info(f"✅ Nenhuma guia GF3I encontrada no banco para o dia {data_alvo}.")
        return
    
    total = len(guias)
    logging.info(f"Encontradas {total} guias GF3I. Avaliando pendências...\n")
    ...
    for index, guia in enumerate(guias, start=1):
        ...
        # Filtro 2 (Python-side): Pula se já tiver todas as informações de enriquecimento
        ...
        if tem_relevante and tem_rotas and tem_especies:
            logging.info(f"[{index}/{total}] ⏭️ Guia {numero} ignorada. Já possui todas as informações (Relevante, Rota, Espécies).")
            ignoradas += 1
            continue

        logging.info(f"[{index}/{total}] Processando Guia pendente: {numero}...")

        # Processamento (Single-Pass)
        estados_da_rota, lista_nomes = processar_pdf_guia(url_pdf)
        rota_to = 'TO' in estados_da_rota

        qtd_especies = guia.get('num_especie')
        if qtd_especies is None:
            qtd_especies = buscar_num_especies(BASE_API_SEMAS, numero, tipo)
        ...
        try:
            ...
            atualizadas += 1
            logging.info(f"   -> Sucesso! Estados: {estados_da_rota} | TO: {rota_to} | Espécies: {len(lista_nomes)}")
        except Exception as e:
            logging.error(f"   -> Erro ao atualizar guia {numero} no banco: {e}")

        time.sleep(1)

    logging.info(f"\n🎉 Varredura do dia {data_alvo} finalizada! {atualizadas} atualizadas, {ignoradas} ignoradas.")

if __name__ == "__main__":
    agora = datetime.now(FUSO_BR)
    logging.info(f"🕒 Iniciando script histórico às {agora.strftime('%H:%M:%S')} (Fuso: Brasília)")
    
    # Loop de histórico para os primeiros 15 dias de setembro (ajuste o range/mês conforme necessário)
    for i in range(1, 6):
        dia = f"{i:02d}"
        data_alvo = f"2026-09-{dia}"
        
        atualizar_historico_guias(data_alvo)
        logging.info("-" * 40)